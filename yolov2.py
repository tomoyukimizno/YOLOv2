...
class YOLOv2(chainer.Chain):
    ...
    def __call__(self, x, t):
        ...
        conf = F.ceil(best_ious - self.thresh) * conf_pred
    ...
    def get_anchor_index(self, width, height, grid_w, grid_h):
        abs_anchors = (self.anchors / cupy.array([grid_w, grid_h])).astype(cupy.float32)
        shape = (self.n_boxes, *width.shape)
        ex_w = F.broadcast_to(width, shape)
        ex_h = F.broadcast_to(height, shape)
        ex_aw = F.broadcast_to(F.reshape(abs_anchors[:, 0], (self.n_boxes, 1, 1)), shape)
        ex_ah = F.broadcast_to(F.reshape(abs_anchors[:, 1], (self.n_boxes, 1, 1)), shape)
