...
class YOLOv2(chainer.Chain):
    ...
    def __init__(self, n_classes, n_boxes):
        super(YOLOv2, self).__init__(
            ...
            conv19=L.Convolution2D(
                1024, 1024, ksize=3, stride=1, pad=1, nobias=True),
            bn19=L.BatchNormalization(
                1024, use_beta=False),
            bias19=L.Bias(shape=(1024,)),
            conv20=L.Convolution2D(
                1024, 1024, ksize=3, stride=1, pad=1, nobias=True),
            bn20=L.BatchNormalization(
                1024, use_beta=False),
            bias20=L.Bias(shape=(1024,)),
            conv21=L.Convolution2D(
                3072, 1024, ksize=3, stride=1, pad=1, nobias=True),
            bn21=L.BatchNormalization(
                1024, use_beta=False),
    ...
    # sum_{画像}{画像 i の bbox} で学習を回す前提
    # 画像 i の bbox の学習が不完全
    # iterator を自作 or あきらめて __call__ 内で for を回すか決める
    def __call__(self, x_in, t, tops, lefts, bottoms, rights):
        # common layer
        h = F.leaky_relu(self.bias1(self.bn1(self.conv1(x_in), test=not self.train)), slope=0.1)
        ...
        x, y, w, h, conf, prob = F.split_axis(
            F.reshape(output, (batch_size, self.n_boxes, self.n_classes + 5, grid_h, grid_w)),
            (1, 2, 3, 4, 5),
            axis=2)
        ...
        """
        x_shift = chainer.Variable(
            np.broadcast_to(
                np.arange(
                    grid_w, dtype=np.float32), x.shape[1:]))
        y_shift = chainer.Variable(
            np.broadcast_to(
                np.arange(
                    grid_h, dtype=np.float32).reshape(grid_h, 1), y.shape[1:]))
        w_anchor = chainer.Variable(
            np.broadcast_to(
                np.reshape(
                    np.array(
                        self.anchors, dtype=np.float32)[:, 0], (self.n_boxes, 1, 1, 1)),
                w.shape[1:]))
        h_anchor = chainer.Variable(
            np.broadcast_to(
                np.reshape(
                    np.array(
        ...
        true_l = chainer.Variable(np.array([np.broadcast_to(chainer.cuda.to_cpu(lefts.data[i]), x.shape[1:]) for i in range(x.shape[0])]))
        true_t = chainer.Variable(np.array([np.broadcast_to(chainer.cuda.to_cpu(tops.data[i]), x.shape[1:]) for i in range(x.shape[0])]))
        # true_r = chainer.Variable(np.array([np.broadcast_to(chainer.cuda.to_cpu(rights.data[i]), x.shape[1:]) for i in range(x.shape[0])]))
        # true_b = chainer.Variable(np.array([np.broadcast_to(chainer.cuda.to_cpu(bottoms.data[i]), x.shape[1:]) for i in range(x.shape[0])]))
        """
        ...
        # loss計算
        tx, ty, tw, th, tconf, tprob = chainer.Variable(tx), chainer.Variable(
            ty), chainer.Variable(tw), chainer.Variable(th), chainer.Variable(
                tconf), chainer.Variable(tprob)
        box_learning_scale, conf_learning_scale = chainer.Variable(
            box_learning_scale), chainer.Variable(conf_learning_scale)
    ...
    def predictor(self, input_x):
        ...
        # x, y, w, hを絶対座標へ変換
        x_shift = chainer.Variable(np.broadcast_to(np.arange(grid_w, dtype=np.float32), x.shape))
        y_shift = chainer.Variable(
            np.broadcast_to(
                np.arange(
                    grid_h, dtype=np.float32).reshape(grid_h, 1), y.shape))
