    ...
    def __call__(self, orig_img):
        ...
        for i in range(int(detected_indices.sum())):
            results.append({
                ...
                "box": Box(x[detected_indices][i] * orig_input_width,
                           y[detected_indices][i] * orig_input_height,
                           w[detected_indices][i] * orig_input_width, h[detected_indices][i] *
                           orig_input_height).crop_region(orig_input_height, orig_input_width)
            })
...
if __name__ == "__main__":
    ...
    for result in nms_results:
        ...
        cv2.putText(orig_img, text, (left, top - 6), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                    (255, 255, 255), 2)
        print(text)
    cv2.imshow("temp", orig_img)
    cv2.imwrite("temp.jpg", orig_img)
    cv2.waitKey(0)
